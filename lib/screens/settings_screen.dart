...
class _VoiceEnrollmentSection extends StatefulWidget {
  ...
  @override
  State<_VoiceEnrollmentSection> createState() => _VoiceEnrollmentSectionState();
}
...
class _VoiceEnrollmentSectionState extends State<_VoiceEnrollmentSection> {
  ...
  Future<void> _checkStatus() async {
    ...
    final user = AuthService.instance.currentUser;
    if (user == null) { setState(() => _state = _EnrollState.idle); return; }
    ...
    try {
      ...
      setState(() { _enrolledAt = null; _state = _EnrollState.idle; });
    }
  }
  ...
  Future<void> _startEnrollment() async {
    ...
    if (!api.isConnected) {
      setState(() { _state = _EnrollState.error; _errorMessage = 'Not connected to server. Set URL in Connections first.'; });
      return;
    }
    ...
    if (!status.isGranted) {
      setState(() { _state = _EnrollState.error; _errorMessage = 'Microphone permission denied.'; });
      return;
    }
    ...
    setState(() { _state = _EnrollState.recording; _countdown = 7; });
    ...
    _countdownTimer = Timer.periodic(const Duration(seconds: 1), (t) async {
      ...
      if (_countdown <= 0) {
        t.cancel();
        await _finishRecording(path, user.id, user.userMetadata?['full_name'] ?? user.email ?? 'User');
      }
    });
  }

  Future<void> _finishRecording(String path, String userId, String userName) async {
    ...
    switch (_state) {
      case _EnrollState.checking:
        return const Padding(
          ...
          child: Row(children: [SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)), SizedBox(width: 12)]),
        );
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Container(width: 40, height: 40, decoration: BoxDecoration(color: AppColors.error.withOpacity(0.15), borderRadius: BorderRadius.circular(10)),
                    child: const Icon(Icons.mic, color: AppColors.error, size: 22)),
                  const SizedBox(width: 14),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Recording...', style: GoogleFonts.manrope(fontSize: 15, fontWeight: FontWeight.w600, color: widget.isDark ? Colors.white : const Color(0xFF0F172A))),
                        Text('Speak naturally for $_countdown more seconds', style: GoogleFonts.manrope(fontSize: 12, color: AppColors.textMuted)),
                      ],
                    ),
                  ),
                  TextButton(onPressed: _cancelRecording, child: Text('Cancel', style: GoogleFonts.manrope(color: AppColors.textMuted, fontSize: 13))),
                ],
              ),
              const SizedBox(height: 8),
      ...
      case _EnrollState.uploading:
        return Padding(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 14),
            Expanded(child: Text('Uploading & verifying enrollment...', style: GoogleFonts.manrope(fontSize: 14, color: AppColors.textMuted))),
      ...
      case _EnrollState.success:
        return Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
          child: Row(children: [
            Container(width: 40, height: 40, decoration: BoxDecoration(color: AppColors.success.withOpacity(0.15), borderRadius: BorderRadius.circular(10)),
              child: const Icon(Icons.check_circle_outline, color: AppColors.success, size: 22)),
            const SizedBox(width: 14),
            Expanded(child: Text('Voice enrolled successfully!', style: GoogleFonts.manrope(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.success))),
          ]),
        );
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Container(width: 40, height: 40, decoration: BoxDecoration(color: AppColors.error.withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
                    child: const Icon(Icons.error_outline, color: AppColors.error, size: 22)),
                  const SizedBox(width: 14),
                  Expanded(child: Text('Enrollment failed', style: GoogleFonts.manrope(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.error))),
                  TextButton(
                    onPressed: () => setState(() => _state = _EnrollState.idle),
                    child: Text('Retry', style: GoogleFonts.manrope(color: AppColors.error, fontWeight: FontWeight.w700, fontSize: 13)),
                  ),
                ],
              ),
              if (_errorMessage != null)
                Padding(
                  ...
                  child: Text(_errorMessage!, style: GoogleFonts.manrope(fontSize: 12, color: AppColors.textMuted), maxLines: 2, overflow: TextOverflow.ellipsis),
                ),
            ],
          ),
        );
              ...
              children: [
                    ...
                    children: [
                      Text('Voice Enrollment',
                        style: GoogleFonts.manrope(fontSize: 15, fontWeight: FontWeight.w500,
                          color: widget.isDark ? Colors.white : const Color(0xFF0F172A))),
                      Text(
                        isEnrolled ? 'Enrolled on ${_formatDate(_enrolledAt)}' : 'Not enrolled — tap to record',
                        style: GoogleFonts.manrope(fontSize: 12,
                          color: isEnrolled ? AppColors.success : AppColors.textMuted),
                      ),
                    ],
                  ),
                ),
                if (isEnrolled)
                  Container(
                    ...
                    child: Text('Active', style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.success)),
                  )
                else
                  Text('Enroll', style: GoogleFonts.manrope(fontWeight: FontWeight.w700, fontSize: 13, color: color)),
                const SizedBox(width: 4),
                Icon(Icons.chevron_right, size: 20, color: widget.isDark ? const Color(0xFF64748B) : Colors.grey.shade400),
              ],
            ),
          ),
        );
